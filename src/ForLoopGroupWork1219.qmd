...
title: For Loop's Group Project
execute:
  echo: false
format:
  html:
...
| --------------  | -------------------- |
| Data collection | Time allocation      |
| Graph production| Data analysis         |

## Priorities for Feedback

The method form data analysis to policy making.
...
Topic 2: Impact of Airbnb on London’s Tourism Economy — Assuming that a certain percentage of tourist money will be spent locally, the extent to which the way in which tourist "money" is spent is likely to have a positive impact on areas with fewer tourists.
...
### 6.1 Data wrangling
In order to classified whether the host of Airbnb is local or not, a cluster analyse is used in the following. However, in the current Airbnb data, when a host has multiple Aribnb listings, the count of multiple listings of a host will be displayed multiple times in the 'calculated_host_listings_count' field in Airbnb's original csv. Therefore, in the pre-processing of clustering data, new dataframes need to be generated by counting according to the id of the landlord, and then the number of housing resources for each landlord is clustered.
...
## loading data
df = pd.read_csv("https://github.com/LiangWeiXian11/fsdsData/raw/main/listings.csv", low_memory=False)
...
Initially, when using k-means cluster to classify the type of Airbnb host, the outcome does not satisfy with our prediction. That is because k-means always tried to minimize the sum of the squares of the errors within the cluster and therefore might sometimes achieve local rather than global optimality. The value of the number of Airbnb listings owned by each host has a large distribution range, and most of the values have a maximum frequecy value between 1 and 3, so it is possible that the Gaussian mixture model based on probability distribution can obtain better clustering effect.
...
plt.title('Figure 1 Airbnbs Counts of different types of hosts')
plt.xlabel('id')
...
fig, ax = plt.subplots(2, 2, figsize=(13, 8), edgecolor='black', linewidth=1)
...
fig.suptitle('Figure 2 Airbnb Map in London')

plt.show()
```

The spatial map above shows the distributions of different types of hosts. For instance, it can be viewed that most of the hosts of London Airbnb are local. In addition, the distribution of Local hosts and company host are similar which is they are gathering near the center of London. Therefore, regulation of Airbnb should be focused on central London.
...
def summarize_room_types():
    ...
    data = pd.read_csv("https://github.com/LiangWeiXian11/fsdsData/raw/main/listings.csv", low_memory=False)
...
The table above shows the percentage of different types of Airbnb in London. It can be observed that the type of "Entire home" owns the highest proportion. As a result, as indicated by several researches, the decrease of long-term rental markets are decreased and the rental prices increase. To reduce the impact of short-term rental to long-term rental, a 90-day limited policy was published which serves to prevent long-term rental properties from being turned into year-round vacation rentals and keeps them accessible for locals. However, there are several discussion on the policy. A variety of stakeholders have expressed support for and opposition to the Airbnb 90-day regulation. Although the regulation aims to address worries about how short-term rentals affect local communities and property markets, hosts and small companies may suffer as a result of it. Therefore, in order to ensure that the 90-day rule and other short-term rental restrictions accomplish their intended purposes without inadvertently harming hosts or local economies, authorities must carefully consider the costs and advantages of enacting them. The effectiveness of laws governing short-term rentals, such as the 90-day Airbnb rule, ultimately rests on finding the ideal balance between fostering tourism and maintaining reasonably priced housing for locals. Policymakers, hosts, and communities will all continue to face difficulties in striking this balance, while the discussion over legislation pertaining to short-term rentals rages on.
...
A LM3 model is applied which assumed that When 1 pound is spent with a local vendor, the local economy benefits by 1.76 pounds, but only by 36 pence when it is spent elsewhere(Source: LM3localmultiplier).
...
#shapefile_path = '../data/London_Borough_Excluding_MHW/London_Borough_Excluding_MHW.shp'
shapefile_path = 'https://github.com/LiangWeiXian11/fsdsData/raw/main/London_Borough_Excluding_MHW/London_Borough_Excluding_MHW.shp'
...
count_points_in_polygon.plot(color=colors, figsize=(9, 5))

plt.title('Figure 3 Borough cluster')
...
expenditure_point = pd.read_csv('https://github.com/LiangWeiXian11/fsdsData/raw/main/updated_data.csv')

# expenditure_point = df_other
...
# make sure GeoDataFrame to use the same CRS
...
# use 'id' columns DataFrame and GeoDataFrame
merged_gdf = non_tourism_borough.merge(df_other, left_on='NAME', right_on = 'neighbourhood')
...
grouped_data = expenditure_non.groupby('GSS_CODE')['tourists_expenditure'].sum().reset_index()
merged_data = borough.merge(grouped_data, on='GSS_CODE')
merged_data['total_money'] = merged_data['tourists_expenditure'] + merged_gdf['local_economy_contribution_from_airbnb']
```

```{python}
merged_data=merged_data.to_crs('epsg:27700')
merged_gdf=merged_gdf.to_crs('epsg:27700')
#  Build a graph with one row three columns
fig, axes = plt.subplots(3, 1, figsize=(8, 14), edgecolor='black', linewidth=1)

#  merged_data -- 'tourists_expenditure' column，'plasma_r' colour
merged_data.plot(column='tourists_expenditure', ax=axes[0], legend=True,
                 cmap='Greens')

#  merged_gdf -- 'local_economy_contribution_from_airbnb'  'Blues' colour
merged_gdf.plot(column='local_economy_contribution_from_airbnb', ax=axes[1], legend=True,
                cmap='Blues')

# merged_data -- 'tourists_expenditure' -- 'plasma_r' 
merged_data.plot(column='total_money', ax=axes[2], legend=True,
                 cmap='plasma_r')

axes[0].set_title('Tourist Expenditure to neighbourhood', fontsize=8)
axes[1].set_title('Host Economic Contribution to neighbourhood', fontsize=8)
axes[2].set_title('Total Economic Contribution of Airbnb', fontsize=8)

axes[0].text(-0.1, 1.09, '(a)', transform=axes[0].transAxes)
axes[1].text(-0.1, 1.09, '(b)', transform=axes[1].transAxes)
axes[2].text(-0.1, 1.09, '(c)', transform=axes[2].transAxes)
...
# add the map title
fig.text(0.5, 0.94, 'Figure 4 Economy Contribution of Airbnb Toutist', ha='center', va='top', fontsize=13)


plt.show()
```

Through the occupancy rate model, tourist consumption model and the spatial distribution of Airbnb, the direct economic contribution of Airbnb tourists to each borough can be estimated, ranging from 1.15 million to 7.79million. Once again, we make a cluster analysis based on tourist spending, which can be divided into regions with high, moderate and low economic contributions. From the spatial distribution, it can be seen that tourists contribute more to the economy of the city center and the borough near the scenic spot, but less to the economy of the suburb.

### 7.5 Possible policy-making based the study：

To address this issue and promote economic activity in less-visited areas, consider implementing a policy that provides financial incentives or grants to Airbnb hosts operating in non-tourist, non-hotspot regions. This could include tax incentives for properties that significantly contribute to the local economy.

Dynamic Tourist Taxation: Implement a tourist tax that varies based on the borough and the time of year, reflecting the varying impact of tourism across the city. This tax could be higher in tourist-heavy periods and areas, and lower in non-tourist areas. 

Data Sharing Agreements: Suggest that local authorities establish agreements with platforms like Airbnb for data sharing to monitor and effectively manage the impact of short-term rentals on housing and the local economy.

### 7.6 Limitation

Deviation between the Estimated model and actual data: In our analysis, 1) we rely on the officially recommended occupancy rate model to estimate annual revenue and other related variables. This may cause bias, mainly because the model cannot fully capture all the complex factors affecting occupancy, such as seasonal changes, market dynamics, competitive environment and rapid changes in customer preferences. 2) We apply a generalized local economic multiplier. Although this is the multiplier officially announced by the UK, as the city and borough change, the multiplier will also change due to different economic and social factors. 3) We used the per traveler local spending amount reported by Airbnb UK insights report. However, due to differences in tourists’ consumption levels and consumption preferences, this amount will vary greatly. Although these models and data are widely accepted by academics and officials, they often deviate from real-world data in practical applications.
limitation:

Inaccuracies in data processing: When processing data, we spatially mapped tourist attractions onto the administrative divisions of city boroughs. A potential issue arises in the spatial mapping of tourist attractions, particularly those located on the demarcation lines between boroughs. This situation may result in the arbitrary assignment of these attractions to one of the adjacent boroughs, consequently introducing a degree of spatial inaccuracies into our dataset.

Limitations of technology and analytical methods: Data science techniques and methods are constantly evolving, but they also have their limitations. Especially when interpreting big data or using complex algorithms, there may be a risk of misunderstanding or misuse.

The map is provided which shows the influence of local economy of non-tourism boroughs from Airbnb. Moreover, it can be viewed that non-tourism boroughs which near the center of city London tends to have higher rate of economy contribution. Therefore, the relationship of high local economy and non-tourism Airbnb is not significant. However, the study indeed give out an overview of local economy contributed from Airbnb. Based on the study, several policies which focused on short-term rental can be adjusted. For example, the 90-days limit policy can be extended for some non-tourism areas because some short-term rental Airbnb can contribute to local economy.

## References

Airbnb, no date b. Airbnb UK insights report. [online] Available at: https://www.airbnbcitizen.com [Accessed 19 Dec 2023].

Alsudais, A., 2021. Incorrect data in the widely used Inside Airbnb dataset. Decision Support Systems, 141, Article 113453. Available at: https://doi.org/10.1016/j.dss.2020.113453 [Accessed 10 Dec 2023].

Cerna Silovska, H. and Kolarikova, J., 2016. Observation and assessment of local economic development with regard to the application of the local multiplier. European Planning Studies, 24(11), pp.1978-1994. Available at: https://doi.org/10.1080/09654313.2016.1233941 [Accessed 1 Dec 2023].

Domanski, B. and Gwosdz, K., 2010. Multiplier Effects in Local and Regional Development. Volume 29(2), pp.27-37. Available at: https://doi.org/10.2478/v10117-010-0012-7 [Accessed 1 Dec 2023].

Dwyer, L., Forsyth, P. and Dwyer, W., 2020. The Economic Contribution of Tourism. In: Tourism Economics and Policy. 1st ed. [ebook] Channel View Publications, pp.239-279. Available at: ISBN 9781845417345 [Accessed 20 Nov 2023].

Falk, M., Larpin, B. and Scaglione, M., 2019. The role of specific attributes in determining prices of Airbnb listings in rural and urban locations. International Journal of Hospitality Management, 83, pp.132-140. Available at: https://doi.org/10.1016/j.ijhm.2019.04.023 [Accessed 26 Nov 2023].

Inside Airbnb, no date a. Available at: http://insideairbnb.com/get-the-data/ [Accessed 18 Dec 2023].

Gelan, A., 2003. Local economic impacts: The British Open. *Annals of Tourism Research*, 30(2), pp.406-425. Available at: https://doi.org/10.1016/S0160-7383(02)00098-1 [Accessed 21 Nov 2023].

Guttentag, D., 2019. Progress on Airbnb: a literature review. Journal of Hospitality and Tourism Technology, 10(4), pp.814-844. Available at: https://doi.org/10.1108/JHTT-08-2018-0075 [Accessed 20 Nov 2023].

LM3 Online. (Accessed November 20, 2023). About LM3 Online. Available at: https://www.lm3online.com/about.

Olsen, W., 2011. Collecting Qualitative Data. In: Data Collection: Key Debates and Methods in Social Research. 1st ed. [ebook] SAGE Publications, pp.31-92. Available at: ISBN 9781446254189 [Accessed 18 Nov 2023].

Pasquinelli, C. and Bellini, N., 2017. Global Context, Policies and Practices in Urban Tourism: An Introduction. In: N. Bellini and C. Pasquinelli, ed., Tourism in the City. 1st ed. Cham: Springer, pp.10-32. Available at: https://doi.org/10.1007/978-3-319-26877-4_1 [Accessed 25 Nov 2023].

Rey, S., Arribas-Bel, D. and Wolf, L.J., 2023. Geographic Data Science with Python. Part II - Spatial Data Analysis. 1st ed. [online] Chapman & Hall. Available at: https://geographicdata.science/book/intro_part_ii.html [Accessed 30 Nov 2023].

Shabrina, Z., Arcaute, E. and Batty, M., 2022. Airbnb and its potential impact on the London housing market. Urban Studies, 59(1), pp.197-221. Available at: https://doi.org/10.1177/0042098020970865 [Accessed 25 Nov 2023].

So, K.K.F., Oh, H. and Min, S., 2018. Motivations and constraints of Airbnb consumers: Findings from a mixed-methods approach. Tourism Management, 67, pp.224-236. Available at: https://doi.org/10.1016/j.tourman.2018.01.009 [Accessed 1 Dec 2023].

Thulin, P., 2015. Local Multiplier and Economic Base Analysis. In: C. Karlsson, M. Andersson and T. Norman, ed., Handbook of Research Methods and Applications in Economic Geography, 1st ed. Northampton, MA: Edward Elgar Publishing, pp.213-234. Available at: https://doi.org/10.4337/9780857932679 [Accessed 25 Nov 2023].
